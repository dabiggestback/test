...
function showView(view) { if (view === 'admin' && !isAdmin) { showAdminLogin(); return; } $$('.nav-item[data-view]').forEach(button => button.classList.toggle('active', button.dataset.view === view)); $$('.view').forEach(section => section.classList.toggle('active-view', section.id === view)); $('.sidebar').classList.remove('open'); }
...
function showAdminLogin() {
  ...
  openModal(`<h3>Admin sign in</h3><p>Use an authorized Google account or administrator email/password to manage school-wide information.</p><button class="google-button" id="googleAdminLogin"><span class="google-g">G</span> Continue with Google</button><div class="or"><span></span><b>or</b><span></span></div><label>Email</label><input id="adminEmail" type="email" autocomplete="username" placeholder="name@school.org"><label>Password</label><input id="adminPassword" type="password" autocomplete="current-password" placeholder="Password"><p class="form-error hidden" id="adminAuthError"></p><button class="primary-button" id="adminLogin">Sign in with email</button>`);
  const showAuthError = message => { const error = $('#adminAuthError'); error.textContent = message; error.classList.remove('hidden'); };
  $('#googleAdminLogin').onclick = async () => {
    try {
      const credential = await signInWithPopup(auth, new GoogleAuthProvider());
      await refreshAdminState(credential.user);
      if (!isAdmin) {
        await signOut(auth);
        showAuthError(`Google sign-in worked, but ${credential.user.email || 'this account'} is not an authorized administrator. Add this account's UID to Firestore /admins first.`);
        return;
      }
      closeModal(); showView('admin'); renderAdmin();
    } catch (error) {
      const code = error?.code || '';
      if (code.includes('operation-not-allowed')) showAuthError('Google sign-in is disabled in Firebase Authentication. Enable the Google provider in Firebase Console → Authentication → Sign-in method.');
      else if (code.includes('unauthorized-domain')) showAuthError('Firebase rejected this website domain. Add knoll.bigback.org under Authentication → Settings → Authorized domains.');
      else if (code.includes('popup-blocked')) showAuthError('The Google sign-in popup was blocked by your browser. Allow popups for this site and try again.');
      else showAuthError(`Google sign-in failed (${code || 'unknown error'}). Check the Firebase Authentication settings.`);
    }
  };
  $('#adminLogin').onclick = async () => {
    ...
    try {
      ...
      await refreshAdminState(credential.user);
      if (!isAdmin) { await signOut(auth); showAuthError('The account signed in successfully, but it is not an authorized administrator.'); return; }
      closeModal(); showView('admin'); renderAdmin();
    } catch (error) {
      const code = error?.code || '';
      if (code.includes('invalid-credential') || code.includes('wrong-password') || code.includes('user-not-found')) showAuthError('That email/password combination was not accepted.');
      else if (code.includes('operation-not-allowed')) showAuthError('Email/password sign-in is disabled in Firebase Authentication.');
      else showAuthError(`Sign-in failed (${code || 'unknown error'}).`);
    }
  };
}

const weatherCodes = { 0: ['☀️', 'Clear sky'], 1: ['🌤️', 'Mostly clear'], 2: ['⛅', 'Partly cloudy'], 3: ['☁️', 'Overcast'], 45: ['🌫️', 'Foggy'], 48: ['🌫️', 'Foggy'], 51: ['🌦️', 'Light drizzle'], 53: ['🌦️', 'Drizzle'], 55: ['🌧️', 'Heavy drizzle'], 61: ['🌦️', 'Light rain'], 63: ['🌧️', 'Rain'], 65: ['🌧️', 'Heavy rain'], 71: ['🌨️', 'Light snow'], 73: ['🌨️', 'Snow'], 75: ['❄️', 'Heavy snow'], 80: ['🌦️', 'Rain showers'], 81: ['🌧️', 'Rain showers'], 82: ['⛈️', 'Heavy showers'], 95: ['⛈️', 'Thunderstorm'], 96: ['⛈️', 'Thunderstorm'], 99: ['⛈️', 'Thunderstorm'] };
async function loadWeather() {
  const location = $('#weatherLocation'), temp = $('#weatherTemp'), condition = $('#weatherCondition'), highLow = $('#weatherHighLow'), iconEl = $('#weatherIcon'), updated = $('#weatherUpdated');
  if (!location || !temp) return;
  try {
    const response = await fetch('https://api.open-meteo.com/v1/forecast?latitude=35.007&longitude=-80.945&current=temperature_2m,weather_code&daily=temperature_2m_max,temperature_2m_min&temperature_unit=fahrenheit&timezone=America%2FNew_York&forecast_days=1', { cache: 'no-store' });
    if (!response.ok) throw new Error(`weather-${response.status}`);
    const weather = await response.json();
    const code = weather.current?.weather_code ?? 0;
    const [icon, label] = weatherCodes[code] || ['🌤️', 'Current conditions'];
    location.textContent = 'Fort Mill, SC';
    temp.textContent = `${Math.round(weather.current.temperature_2m)}°`;
    condition.textContent = label;
    highLow.textContent = `H ${Math.round(weather.daily.temperature_2m_max[0])}° · L ${Math.round(weather.daily.temperature_2m_min[0])}°`;
    iconEl.textContent = icon;
    updated.textContent = `Updated ${new Date().toLocaleTimeString([], { hour: 'numeric', minute: '2-digit' })} · Open-Meteo`;
  } catch {
    condition.textContent = 'Weather unavailable';
    highLow.textContent = 'Try again later';
    updated.textContent = 'Could not reach the weather service.';
  }
}
